/* ============================================
   PANEL ALUMNO - COMPLETO Y FUNCIONAL
   Conectado a backend real con UX premium
   ============================================ */
...
const PanelAlumno = {
  ...
  ubicacionCentro: { lat: 42.2406, lng: -8.7207 }, // Coordenadas IES de Teis, Vigo
  ...
  async cargarDashboard() {
    try {
      ...
      this.renderTurnosActivos(data);
      this.renderDocumentosPendientes(data);
  ...
  async cargarNotificaciones() {
    try {
      ...
      if (window.Header && typeof Header.updateNotifications === 'function') {
        Header.updateNotifications(noLeidas);
      }

      // Actualizar badge en notificaciones del contador
  ...
  renderTurnosActivos(data) {
    const container = document.getElementById('turnos-activos-list');
    if (!container) return;

    if (!data.turnosActivos || data.turnosActivos.length === 0) {
      container.innerHTML = '<p class="text-secondary text-center" style="padding: 24px;">No tienes turnos activos</p>';
      return;
    }

    container.innerHTML = data.turnosActivos.slice(0, 5).map(turno => `
      <div class="item-list">
        <div class="item-icon">🎫</div>
        <div class="item-content">
          <p class="item-title">Turno #${turno.numeroTurno}</p>
          <p class="item-subtitle">${this.formatDateTime(turno.fechaCita)}</p>
        </div>
        <span class="badge badge-${this.getBadgeColor(turno.estadoTurno)}">${turno.estadoTurno}</span>
      </div>
    `).join('');
  ...
  renderDocumentosPendientes(data) {
    ...
    if (!data.documentosPendientes || data.documentosPendientes.length === 0) {
      container.innerHTML = '<p class="text-secondary text-center" style="padding: 24px;">✅ No tienes documentos pendientes</p>';
      return;
    }

    container.innerHTML = data.documentosPendientes.slice(0, 5).map(doc => `
      <div class="item-list">
        <div class="item-icon">📄</div>
        <div class="item-content">
          <p class="item-title">${doc.nombreDocumento || 'Documento'}</p>
          <p class="item-subtitle">Estado: ${doc.estadoDocumento}</p>
        </div>
      </div>
    `).join('');
  },
  ...
  initHeroDinamico() {
    ...
    heroMessage.innerHTML = `${saludo} ${emoji}<br>Todo lo que necesitas, en un solo lugar.`;
  },
  ...
  initUbicacion() {
    const ubicacionBtn = document.getElementById('btn-usar-ubicacion');
    if (ubicacionBtn) {
      ubicacionBtn.addEventListener('click', () => this.obtenerUbicacion());
    }
  },
  ...
  async obtenerUbicacion() {
    ...
    if (btnUbi) {
      ...
      btnUbi.innerHTML = '<div class="spinner" style="width: 16px; height: 16px;"></div> Obteniendo...';
    }
    ...
    try {
      ...
      if (distanciaElement) {
        ...
        distanciaElement.innerHTML = `
          <div class="ubicacion-info ${dentroRadio ? 'dentro-radio' : 'fuera-radio'}">
            <div class="ubicacion-icon">${dentroRadio ? '✅' : '📍'}</div>
            <div class="ubicacion-text">
              <strong>${Math.round(distancia)}m</strong> del centro<br>
              <span style="font-size: 0.875rem; opacity: 0.9;">
                ${dentroRadio ? 'Estás cerca, puedes hacer check-in' : 'Acércate más para hacer check-in'}
              </span>
            </div>
          </div>
        `;
      ...
      if (distanciaElement) {
        distanciaElement.innerHTML = `
          <p class="text-secondary">No se pudo obtener tu ubicación. Permite el acceso en tu navegador.</p>
        `;
      }
    } finally {
      if (btnUbi) {
